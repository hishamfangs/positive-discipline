  ...
  function updateActiveLink() {
    ...
    linkMap.forEach(({ section }) => {
      ...
      if (viewportMidpoint >= sectionTop && viewportMidpoint <= sectionBottom) {
        const distanceToMid = Math.abs(viewportMidpoint - (sectionTop + section.offsetHeight / 2));
        if (distanceToMid < smallestDistance) {
          smallestDistance = distanceToMid;
          candidate = section;
        }
      } else if (!candidate) {
        const distanceToTop = Math.abs(viewportMidpoint - sectionTop);
...
function initPriceCalculators() {
  ...
  const basePriceEGP = 24000; // Base price per person in EGP
